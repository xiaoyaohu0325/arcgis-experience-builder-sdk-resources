...
export default class RowItem extends React.PureComponent<LayoutItemProps & OwnProps, State> {
  ...
  render() {
    ...
    const mergedClass = classNames('row-layout-item', `col-${span}`, `offset-${offset}`, `order-${order}`, {
      'fix-height': (!itemSetting.autoProps?.height && heightMode !== 'fit') || itemSetting.autoProps?.height
    });
    ...
    return (
      <LayoutItemInBuilder
        ...
        right={true}
        top={false}
        bottom={heightMode === 'fixed' && !itemSetting.autoProps?.height}
        draggable={draggable}
        resizable={resizable}
        selectable={selectable}
        onClick={this.props.onClick}
        className={mergedClass}
        forceAspectRatio={heightMode === 'ratio' && !itemSetting.autoProps?.height}
        aspectRatio={ratio}>
        <div>
